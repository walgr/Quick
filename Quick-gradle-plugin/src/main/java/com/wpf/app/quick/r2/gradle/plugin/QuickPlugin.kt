...
class QuickPlugin : Plugin<Project> {
    ...
    private fun getPackageName(variant: BaseVariant): String {
        val xmlSlurped = XmlSlurper(false, false)
        ...
        val result = xmlSlurped.parse(list[0])
            ...
            variant.outputs.all { output ->
                ...
                if (once.compareAndSet(false, true)) {
                    ...
                    val generate = project.tasks.register(
                        "generate${variant.name.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }}R2",
                        R2Generator::class.java
                    ) {
