...
fun <F: View, T: View, B: View>ViewGroup.coordinator(
    ...
    followSlideLayout?.invoke(appBarLayout)?.let {
        it.layoutParams.forceTo<AppBarLayout.LayoutParams>().scrollFlags = scrollFlags!!
        followSlideLayoutView = it
        if (it.parent() != appBarLayout) {
            appBarLayout.addView(it, matchWrapLayoutParams)
        }
        topSuspendLayout?.invoke(appBarLayout)?.let { topSuspendLayout ->
            topSuspendLayoutView = topSuspendLayout
            if (topSuspendLayout.parent() != appBarLayout) {
                appBarLayout.addView(topSuspendLayout, matchWrapLayoutParams)
            }
        }
    }
    var bottomScrollLayoutView: B
    bottomScrollLayout.invoke(parentLayout).let {
        it.layoutParams.forceTo<CoordinatorLayout.LayoutParams>().behavior = behavior
